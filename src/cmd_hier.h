...
#define GET_PARAM_LEAF(param)    \
      (param->cmd_type.leaf)

#define IS_PARAM_NO_CMD(param)   \
      (param->param_type == NO_CMD)

#define IS_PARAM_CMD(param)      \
      (param->param_type == CMD)

#define IS_PARAM_LEAF(param)     \
      (param->param_type == LEAF)

#define GET_LEAF_TYPE_STR(param) \
      (get_str_leaf_type(GET_PARAM_LEAF(param)->leaf_type))

#define GET_LEAF_VALUE_PTR(param)  \
      (GET_PARAM_LEAF(param)->value_holder)

#define GET_LEAF_TYPE(param)      \
     (GET_PARAM_LEAF(param)->leaf_type)

#define GET_CMD_NAME(param)       \
     (GET_PARAM_CMD(param)->cmd_name)

#define GET_PARAM_HELP_STRING(param)  \
    (param->help)

#define GET_LEAF_ID(param)  \
       (GET_PARAM_LEAF(param)->leaf_id)
...
param_t *
get_current_branch_hook(param_t *);
void
reset_cmd_tree_cursor();
void
goto_top_of_cmd_tree(param_t *);
void
go_one_level_up_cmd_tree(param_t *);
void
set_cmd_tree_cursor(param_t *);
param_t *
get_cmd_tree_cursor();
param_t*
find_matching_param(param_t **, const char *);
void
build_mode_console_name(param_t *);
void
